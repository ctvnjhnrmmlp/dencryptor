	...
	while (ip < text.length) {
		...
		for (let i = 0; i < alphabet.length; i++) {
			...
			if (character == alphabet[i]) {
				if (random_input == 1) decryptedText += crypts[1][alphabet[i]];
				else if (random_input == 2) decryptedText += crypts[2][alphabet[i]];
				else if (random_input == 3) {
					if (crypts[3][alphabet[i]] == undefined) {
						decryptedText += crypts[2][alphabet[i]];
						break;
					}

					decryptedText += crypts[3][alphabet[i]];
				} else if (random == 4) {
					...
					decryptedText += crypts[4][alphabet[i]];
				}
